...
export async function migrateFromLocalStorage(): Promise<void> {
  try {
    ...
    const existingCalculations = localStorage.getItem('metal-calculations')
    if (existingCalculations) {
      const calculations: Calculation[] = JSON.parse(existingCalculations).map((calc: any) => ({
        ...calc,
        timestamp: new Date(calc.timestamp),
        projectId: undefined // Will be assigned to projects later
      }))

      // Create a default "General" project for existing calculations
      const generalProjectId = await createProject({
        name: 'General Calculations',
        description: 'Migrated calculations from previous version',
      ...
      // Save calculations with project reference
      for (const calc of calculations) {
        const calcWithProject = { ...calc, projectId: generalProjectId }
        await createRecord(STORES.CALCULATIONS, calcWithProject)
      }

      // Update project with calculation IDs
      const project = await getProject(generalProjectId)
      if (project) {
        project.calculationIds = calculations.map(c => c.id)
        await updateProject(project)
      }

      console.log(`Migrated ${calculations.length} calculations to IndexedDB`)
    }
    ...
    if (userPreferences) {
      ...
      await saveSetting('user-preferences', prefs)
      console.log('Migrated user preferences to IndexedDB')
    }

    // Mark migration as complete
    await saveSetting('migration-complete', true)
    await saveSetting('migration-date', new Date().toISOString())
...
export async function isMigrationNeeded(): Promise<boolean> {
  try {
    const migrationComplete = await getSetting('migration-complete')
